...
def setup_train_log():
    ...
    log_path = result_dir / f"train_validator_{timestamp}.log"
...
def main():
    ...
    with ProcessPoolExecutor(max_workers=min(max(1, int(ns.num_workers)), len(symbols))) as ex:
        jobs = [
            ex.submit(
        ...
        for fut in as_completed(jobs):
            ...
            print(f"[完成] {symbol}: events={len(res['events'])}, labeled={len(labeled)}")
