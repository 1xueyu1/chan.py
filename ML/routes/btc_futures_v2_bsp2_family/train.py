...
from .candidate_family import BSP2_FAMILY_ORDER, attach_bsp2_candidate_family
...
BSP_FAMILIES = BSP2_FAMILY_ORDER
...
DROP_FEATURE_EXACT = {
    ...
    "label_bsp2_exit_reference_level",
    "bsp2_candidate_family",
    "bsp2_family",
}
...
def _candidate_features(train: pd.DataFrame, min_feature_coverage: float) -> list[str]:
    all_features = infer_feature_columns(
        train,
...
def train_btc_futures_models(
    ...
    if train_decision_models:
        metrics["decision_models"] = train_bsp2_decision_models(
            dataset,
    ...
    for side, is_buy_value in (("buy", True), ("sell", False)):
        ...
        for family_idx, family in enumerate(BSP_FAMILIES):
            ...
            selected, report = _select_top_features(
                ...
                random_state=1000 + int(family_idx) * 10 + (0 if side == "buy" else 1),
            )
            ...
            bundle = train_classifier(
                ...
                threshold=0.99,
                side=side,
                random_state=2000 + int(family_idx) * 10 + (0 if side == "buy" else 1),
            )
        ...
        aggregate = SideBspFamilyBundle(side=side, family_bundles=side_bundles, route="btc_futures_v2_bsp2_family")
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Train BTC futures v2 BSP2-family split models")
    ...
    result = train_btc_futures_models(
        ...
        pure_bsp2_only=args.pure_bsp2_only,
