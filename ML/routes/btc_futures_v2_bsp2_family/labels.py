...
BSP2_REALTIME_PATH_TO_ID = {name: idx for idx, name in enumerate(BSP2_REALTIME_PATH_CLASSES)}
...
def label_bsp2_structure_with_1m_path(
    ...
    events = attach_bsp2_candidate_family(events)
    ...
    for _, event in events.iterrows():
        ...
        row: dict[str, object] = {
            ...
            "label_bsp2_family_valid": np.nan,
            "label_bsp2_follow_class": "",
            "label_bsp2_family_exit_mode": "",
            "label_bsp2_initial_invalid_price": np.nan,
            "label_bsp2_t1_price": np.nan,
            "label_bsp2_t2_price": np.nan,
            "label_bsp2_t3_price": np.nan,
            "label_bsp2_trailing_anchor_price": np.nan,
            "label_bsp2_exit_reference_level": "15m",
            "label_bsp2_expected_exit_reason": "",
            "label_bsp2_identity_strict": np.nan,
            "label_pre_confirm_invalid": np.nan,
            "label_pre_confirm_invalid_time": np.nan,
            "label_pre_confirm_invalid_loss": np.nan,
            "label_bsp2_realtime_path": "",
            "label_bsp2_realtime_path_id": np.nan,
            "bsp2_candidate_family": str(event.get("bsp2_candidate_family", "other")),
            "bsp2_family": str(event.get("bsp2_family", event.get("bsp2_candidate_family", "other"))),
        }
        ...
        if invalid_pos is not None:
            follow_path = "invalid"
            follow_class = "invalid_before_third"
            exit_mode = "invalid_stop"
            expected_exit_reason = "structure_invalid"
        elif strong_2_3_combo > 0.0:
            follow_path = "bsp2_t3_overlap"
            follow_class = "bsp2_t3_overlap_strong"
            exit_mode = "trend_follow_opposite_bsp"
            expected_exit_reason = "same_level_opposite_bsp"
        elif return_level >= 3.0:
            follow_path = "return_origin_zs_full"
            follow_class = "return_origin_zs_full"
            exit_mode = "return_zs_target"
            expected_exit_reason = "origin_zs_far_boundary_or_opposite_bsp"
        elif return_prev_zs > 0.0:
            follow_path = "return_origin_zs"
            follow_class = "return_origin_zs"
            exit_mode = "return_zs_target"
            expected_exit_reason = "origin_zs_mid_or_opposite_bsp"
        elif third_move > 0.0 and position_vs_origin_zs in expansion_positions:
            follow_path = "higher_level_expansion_candidate"
            follow_class = "higher_level_expansion"
            exit_mode = "higher_level_zs_protect"
            expected_exit_reason = "higher_level_opposite_bsp"
        elif third_move > 0.0:
            follow_path = "third_move_only"
            follow_class = "new_zs_or_trend"
            exit_mode = "trend_follow_opposite_bsp"
            expected_exit_reason = "same_level_opposite_bsp"
        else:
            follow_path = "weak_no_third_move"
            follow_class = "weak_no_confirm"
            exit_mode = "invalid_stop"
            expected_exit_reason = "weak_timeout_or_structure_invalid"
        if invalid_pos is not None:
        ...
        if invalid_pos is not None:
            realtime_path = "invalid_before_confirm"
        elif strong_2_3_combo > 0.0:
            realtime_path = "bsp2_t3_overlap_trend"
        elif third_move > 0.0 and position_vs_origin_zs in expansion_positions:
            realtime_path = "higher_level_expansion"
        elif third_move > 0.0:
            realtime_path = "third_confirmed"
        elif return_prev_zs > 0.0:
            realtime_path = "return_origin_zs"
        else:
            realtime_path = "weak_no_confirm"
        risk_loss = float(direction * (invalid_price - entry_price) / (entry_price + 1e-12) - cost) if np.isfinite(invalid_price) else np.nan
        strict_family = str(event.get("bsp2_candidate_family", "other") or "other")
        strict_identity = (
            invalid_pos is None
            and has_direct_origin_zs
            and invalid_source == "chan_actual"
            and strict_family in {"standard_bsp2", "bsp2_near_origin_zs_boundary", "bsp2_t3_overlap"}
            and (third_move > 0.0 or return_prev_zs > 0.0)
            and strength_class not in {"weak_no_confirm", "no_origin_zs", "invalid"}
        )
        row.update(
            {
                ...
                "label_bsp2_family_valid": float(1.0 if (invalid_pos is None and follow_class not in {"weak_no_confirm"}) else 0.0),
                "label_bsp2_follow_class": follow_class,
                "label_bsp2_family_exit_mode": exit_mode,
                "label_bsp2_initial_invalid_price": invalid_price,
                "label_bsp2_t1_price": float(zs_low if is_buy else zs_high) if np.isfinite(zs_low) and np.isfinite(zs_high) else np.nan,
                "label_bsp2_t2_price": float(zs_mid) if np.isfinite(zs_mid) else np.nan,
                "label_bsp2_t3_price": float(zs_high if is_buy else zs_low) if np.isfinite(zs_low) and np.isfinite(zs_high) else np.nan,
                "label_bsp2_trailing_anchor_price": float(third_move_ref) if np.isfinite(third_move_ref) else np.nan,
                "label_bsp2_exit_reference_level": "1h" if follow_class == "higher_level_expansion" else "15m",
                "label_bsp2_expected_exit_reason": expected_exit_reason,
                "label_bsp2_identity_strict": float(1.0 if strict_identity else 0.0),
                "label_pre_confirm_invalid": float(1.0 if invalid_pos is not None else 0.0),
                "label_pre_confirm_invalid_time": (
                    float((times[invalid_pos] - times[entry_pos]).total_seconds() / 60.0)
                    if invalid_pos is not None
                    else np.nan
                ),
                "label_pre_confirm_invalid_loss": float(risk_loss) if invalid_pos is not None and np.isfinite(risk_loss) else np.nan,
                "label_bsp2_realtime_path": realtime_path,
                "label_bsp2_realtime_path_id": float(BSP2_REALTIME_PATH_TO_ID.get(realtime_path, -1)),
            }
        )
        rows.append(row)
