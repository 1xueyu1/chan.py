...
class SideBspFamilyBundle:
    ...
    route: str = "btc_futures_v1"
    extra_feature_columns: list[str] = field(default_factory=lambda: ["bsp_type", "bsp_types_str"])
    ...
    def predict_model_key(self, frame: pd.DataFrame) -> np.ndarray:
        families = bsp_family_for_frame(frame)
        return np.asarray([f"v1_bsp{family}_{self.side}" for family in families], dtype=object)
