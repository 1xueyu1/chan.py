...
PRIMARY_LABEL_COLUMN = "label_bsp2_chan_entry_quality"
...
def run_btc_futures_backtest(
    ...
    enable_structure_risk_sizing: bool = False,
    target_trade_risk_pct: float = 0.01,
    min_stake_multiplier: float = 0.20,
    ...
    if exit_mode not in {"post_management", "fixed_label", "structure_target", "opposite_bsp", "third_then_opposite_bsp", "third_then_opposite_no_after_timeout"}:
        raise ValueError("exit_mode must be post_management, fixed_label, structure_target, opposite_bsp, third_then_opposite_bsp, or third_then_opposite_no_after_timeout")
    ...
    if exit_mode == "structure_target" and require_origin_zs:
        ...
        if missing_origin_cols:
            raise ValueError(
                ...
                f"missing columns: {missing_origin_cols}. Rebuild ML.routes.btc_futures_v1.features first, "
    ...
    if pure_bsp2_only or exit_mode == "structure_target":
        ...
        dataset = dataset.loc[bsp_family_for_frame(dataset).astype(str) == "2"].copy()
    ...
    opposite_exit_events = (
        _build_all_bsp_exit_events(dataset, begin, end, tail_days=int(np.ceil(float(no_timeout_max_holding_minutes) / 1440.0)) + 7)
        if exit_mode in OPPOSITE_BSP_EXIT_MODES
    ...
    if not qualified.empty:
        ...
        for _, row in qualified.iterrows():
            ...
            trade = dict(row)
            if exit_mode in {"post_management", "structure_target", *OPPOSITE_BSP_EXIT_MODES}:
                symbol = normalize_symbol(str(row.get("symbol", SYMBOL) or SYMBOL))
                if symbol not in bars_cache:
                    ...
                    tail_minutes = max(max_holding_minutes_window, int(no_timeout_max_holding_minutes) if exit_mode in OPPOSITE_BSP_EXIT_MODES else max_holding_minutes_window)
    ...
    metrics = {
        ...
        "model_design": "BSP2 Chan-original quality: pure BSP2; third same-direction move before structural invalidation; long/short split",
        "exit_mode": str(exit_mode),
        "signal_filter": {
            "pure_bsp2_only": bool(pure_bsp2_only or exit_mode in {"structure_target", "opposite_bsp"}),
            ...
            "bsp_type_rule": "bsp_types_str == '2'" if (pure_bsp2_only or exit_mode in {"structure_target", "opposite_bsp"}) else "bsp_family == '2'",
        },
        "structure_target": {
        ...
        "opposite_bsp_exit": {
            "enabled": exit_mode in OPPOSITE_BSP_EXIT_MODES,
            "require_third_before_opposite": exit_mode in {"third_then_opposite_bsp", "third_then_opposite_no_after_timeout"},
            "no_timeout_after_third": exit_mode == "third_then_opposite_no_after_timeout",
            "no_timeout_before_third": bool(disable_before_third_timeout),
            "cost_protect_after_third": exit_mode == "third_then_opposite_no_after_timeout",
            ...
            "event_pool_size": int(len(opposite_exit_events)) if exit_mode in OPPOSITE_BSP_EXIT_MODES else 0,
            "found": int(trades_df["opposite_bsp_found"].fillna(False).astype(bool).sum()) if not trades_df.empty and "opposite_bsp_found" in trades_df else 0,
...
def main() -> None:
    ...
    parser.add_argument("--exit-mode", default="post_management", choices=["post_management", "fixed_label", "structure_target", "opposite_bsp", "third_then_opposite_bsp", "third_then_opposite_no_after_timeout"])
    ...
    parser.add_argument("--min-stake-multiplier", type=float, default=0.20)
    ...
    metrics = run_btc_futures_backtest(
        ...
        pure_bsp2_only=(args.pure_bsp2_only or not args.include_bsp2s_family),
        ...
        enable_structure_risk_sizing=bool(args.enable_structure_risk_sizing),
