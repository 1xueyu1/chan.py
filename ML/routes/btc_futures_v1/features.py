...
DEFAULT_CACHE_DIR = Path("data/btc_futures_v1/cache")
...
def _feature_cache_path(cache_dir: str | Path, symbol: str) -> Path:
    return Path(cache_dir) / f"btc_futures_v1_features_{normalize_symbol(symbol).lower()}.parquet"
...
def build_btc_futures_dataset(
    ...
    for symbol in requested_symbols:
        ...
        if refresh_labels_only and not cache_path.exists():
            ...
            legacy_path = Path(f"data/btc_futures_v1/btc_futures_v1_dataset_{symbol_asset(symbol).lower()}_only.parquet")
        ...
        tasks.append(
            {
                ...
                "part_path": str(part_root / f"btc_futures_v1_labeled_{symbol.lower()}.parquet"),
    ...
    metadata = {
        ...
        "label_design": (
            "Chan-original BSP2 label: a positive sample requires the third same-direction move "
            "to appear before actual structural invalidation. Return-to-origin-ZS and 2/3 overlap "
            "fields are kept as diagnostics, not the semantic definition of BSP2."
        ),
    ...
    output.with_name("btc_futures_v1_dataset_meta.json").write_text(
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Build futures v1 ML dataset")
