...
from .backtest import run_btc_futures_v2_stable_backtest
from .dataset import DEFAULT_DATASET_PATH, ROUTE_NAME
from .train import train_btc_futures_v2_stable_models


DEFAULT_MODEL_ROOT = Path("result/ml/btc_futures_v2_stable_walk_forward")
DEFAULT_OUTPUT_ROOT = Path("result/btc_futures_v2_stable_walk_forward")
...
def _fold_row(year: int, train_metrics: dict[str, Any], backtest_metrics: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "buy_threshold": float(buy.get("primary_threshold", np.nan)),
        "sell_threshold": float(sell.get("primary_threshold", np.nan)),
        "buy_valid_auc": float((buy.get("valid") or {}).get("auc", np.nan)),
        "sell_valid_auc": float((sell.get("valid") or {}).get("auc", np.nan)),
        "trades": int(backtest_metrics.get("trades", 0)),
        "qualified_signals": int(backtest_metrics.get("qualified_signals", 0)),
        "win_rate": float(backtest_metrics.get("win_rate", np.nan)),
        "total_return": float(backtest_metrics.get("total_return", 0.0)),
        "profit_factor": float(backtest_metrics.get("profit_factor", np.nan)),
        "max_drawdown": float(backtest_metrics.get("max_drawdown", 0.0)),
        "final_equity": float(backtest_metrics.get("final_equity", np.nan)),
    }


def _write_summary_markdown(summary: dict[str, Any], table: pd.DataFrame, path: Path) -> None:
    lines = [
        "# BTC 合约 v2 Stable 年度 Walk-Forward",
        "",
        "## 口径",
        "",
        "- 每个测试年只使用之前的数据训练。",
        "- 测试年前一年做验证集，验证集负责阈值、市场状态分组和 gate 门槛。",
        "- 验证不过成本线的方向或市场状态直接禁用。",
        "- 测试年不参与训练和调参。",
        "",
        "## 汇总",
        ...
        "## 分年结果",
        "",
        "| 年份 | 交易数 | 胜率 | 收益 | 最大回撤 | PF |",
        "|---:|---:|---:|---:|---:|---:|",
    ]
    for _, row in table.iterrows():
        pf = row["profit_factor"]
        pf_text = "inf" if np.isinf(pf) else f"{pf:.3f}" if np.isfinite(pf) else "N/A"
        win = f"{row['win_rate']:.2%}" if np.isfinite(row["win_rate"]) else "N/A"
        lines.append(
            f"| {int(row['test_year'])} | {int(row['trades'])} | {win} | "
            f"{row['total_return']:.2%} | {row['max_drawdown']:.2%} | {pf_text} |"
        )
...
def run_yearly_walk_forward(
    ...
    dataset = pd.read_parquet(dataset_path, columns=["exec_time"])
    ...
    for year in range(int(first_test_year), final_year + 1):
        ...
        print(
            f"[WF2] year={year} train=[{train_start},{valid_start}) "
            f"valid=[{valid_start},{test_start}) test=[{test_start},{test_end_exclusive})",
            flush=True,
        )
        train_metrics = train_btc_futures_v2_stable_models(
        ...
        print(f"[WF2] done year={year} trades={row['trades']} win_rate={win_text} return={row['total_return']:.2%}", flush=True)
    ...
    (output_root_path / "walk_forward_summary.json").write_text(json.dumps(summary, ensure_ascii=False, indent=2), encoding="utf-8")
    _write_summary_markdown(summary, table, output_root_path / "WALK_FORWARD_SUMMARY.md")
    return summary
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Run yearly walk-forward for BTC futures v2 stable")
